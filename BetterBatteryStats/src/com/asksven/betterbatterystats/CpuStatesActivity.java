...
public class CpuStatesActivity extends ListActivity
{
	/**
	 * The logging TAG
	 */
	private static final String TAG = "CpuStatesActivity";
...
//    	this.setListViewAdapter();
	...
	private class LoadStatData extends AsyncTask<Context, Integer, StatsAdapter>
	{
	    ...
	    protected StatsAdapter doInBackground(Context... params)
	    {
			...
			SharedPreferences sharedPrefs = PreferenceManager.getDefaultSharedPreferences(CpuStatesActivity.this);
			boolean bFilter = sharedPrefs.getBoolean("filter_data", true);

			try
			{
				m_listViewAdapter = new StatsAdapter(CpuStatesActivity.this,
						StatsProvider.getInstance(CpuStatesActivity.this).getCpuStateList(BatteryStatsTypes.STATS_CURRENT));
			}
			catch (Exception e)
			{
				Log.e(TAG, "Loading of cpu states stats failed");
		...
		protected void onPostExecute(StatsAdapter o)
	    {
	    	...
	    	CpuStatesActivity.this.setListAdapter(o);
	    }
	    @Override
	    protected void onPreExecute()
	    {
	    	...
	    	if (m_progressDialog == null)
	    	{
		    	m_progressDialog = new ProgressDialog(CpuStatesActivity.this);
