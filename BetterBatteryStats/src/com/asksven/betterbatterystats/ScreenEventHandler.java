    ...
    public void onReceive(Context context, Intent intent)
    {
        ...
        if (intent.getAction().equals(Intent.ACTION_USER_PRESENT))
		{
			...
			try
			{
				...
				if (bIssueWarnings)
				{
					Toast.makeText(context, "Watchdog processing...", Toast.LENGTH_SHORT).show();			
					...
					if (screenOffDurationMs >= ((long)minScreenOffDurationMin*60*1000))
					{
						...
						if (stats.hasScreenOffRef())
						{
							...
							if (awakePct >= awakeThresholdPct)
							{
								...
								i.putExtra(StatsActivity.STAT, 2);
								i.putExtra(StatsActivity.STAT_TYPE, StatsProvider.STATS_SCREEN_OFF);

						    	PendingIntent contentIntent = PendingIntent.getActivity(context, 0, i, 0);

						    	notification.setLatestEventInfo(
						    			context, context.getText(R.string.app_name), awakePct + "% awake while screen off", contentIntent);
